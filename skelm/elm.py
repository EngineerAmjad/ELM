    ...
    def _make_slfns(self, X) -> Iterable[SLFN]:
        ...
        if not hasattr(self.n_neurons, '__iter__'):
            slfn = HiddenLayer(n_neurons=self.n_neurons, density=self.density, ufunc=self.ufunc,
                               pairwise_metric=self.pairwise_metric, random_state=self.random_state,weights = self.weights)
            slfn.fit(X)
            SLFNs.append(slfn)
        ...
        # several different types of neurons
            ...
            weightss = self.weights
            if weightss is None or isinstance(weightss, str):
                weightss = [weightss] * k

            if not k == len(ufuncs) == len(densities) == len(pw_metrics) == len(weightss):
            ...
            for n_neurons, ufunc, density, metric,weights in zip(self.n_neurons, ufuncs, densities, pw_metrics,weightss):
                slfn = HiddenLayer(n_neurons=n_neurons, density=density, ufunc=ufunc,weights =weights,
...
class ELMClassifier(ScikitELM, ClassifierMixin):
    ...
    def __init__(self, classes=None, alpha=1e-7, batch_size=None, include_original_features=False, n_neurons=None,
                 ufunc="tanh", density=None, pairwise_metric=None, random_state=None,weights=None):
